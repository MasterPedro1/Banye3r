...
public class Reconocimiento : MonoBehaviour
{
    ...
    public void MovePlayer()
    {
        ...
        Vector3Int cellPosition = ts.reconocimiento.WorldToCell(new Vector3(mousePosition.x, mousePosition.y, 0f));
        Goal = cellPosition;
    ...
    async void Move()
    {
        ...
        {
            ts.reconocimiento.SetTile(_pathNodes[i], playerSprite);
            ts.reconocimiento.SetTransformMatrix(_pathNodes[i], Matrix4x4.TRS(offset, Quaternion.Euler(0, 0, 0), Vector3.one));
            await Task.Delay(1000);
            if (i != 0)
            {
                ts.reconocimiento.ClearAllTiles();
            }
        }
    }
